...
class Bpn_penerimaan extends CI_Controller
{
    private $module = 'bpn';
    private $controller = 'bpn_penerimaan';
    private $current = 'bpn';
    ...
    public function index()
    {
        ...
        $this->load->view('vbpn_penerimaan', $data);
    }
    ...
    function grid()
    {
        ...
        $this->datatables->select('v.id as validasi_id, {rownum} as rownum,
            get_sspdno(ss.id) as sspdno, ss.tgl_transaksi, get_nop_sspd(ss.id,true) as nop,
            ss.thn_pajak_sppt, ss.wp_nama, coalesce(b.bayar,0) as bayar,
        ...
        if($proses_id == 1) {
            ...
            $this->datatables->where("v.bpn_tgl_terima is null");
        } else {
            if($this->input->get('sSearch') == '')
    ...
    public function terima()
    {
        ...
        if ($id && $get = $this->validasi_model->get($id)) {
            ...
            if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE) {
                ...
                $this->load->view('vbpn_penerimaan_form_banjar', $data);
            } else
                $this->load->view('vbpn_penerimaan_form', $data);
        } else {
            show_404();
        }
    }
    ...
    public function do_terima()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $this->validasi_model->update($id, $post_data);
