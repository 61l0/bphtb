...
class Sspd_approval extends CI_Controller
{
    private $module = 'sspd';
    private $controller = 'sspd_approval';
    ...
    public function index()
    {
        ...
        $this->load->view('vsspd_approval', $data);
    }
    ...
    function grid()
    {
        ...
        $this->datatables->select('coalesce(sa.id, ss.id) id, {rownum} as rownum,
            get_sspd_approvalno(sa.id) as approvalno, sa.tgl_approval,
            get_nop_sspd(ss.id,true) as nop, ss.thn_pajak_sppt, ss.wp_nama,
        ...
        $this->datatables->join('bphtb_berkas_in_det bid', 'bid.sspd_id=ss.id');
        $this->datatables->join('bphtb_sspd_approval sa', 'sa.sspd_id=ss.id', 'left');

        if($proses_id == 1) {
            if($this->input->get('sSearch') == '')
                $this->datatables->filter("date(ss.tgl_transaksi) between '{$tgl1}' and '{$tgl2}'");
            $this->datatables->where("sa.id is null");
        } else {
            if($this->input->get('sSearch') == '')
                $this->datatables->filter("date(sa.tgl_approval) between '{$tgl1}' and '{$tgl2}'");
            $this->datatables->where("sa.id is not null");
        }
    ...
    public function approve()
    {
        ...
        if ($id && $get = $this->sspd_model->get($id)) {
            ...
            if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE) {
                ...
                $this->load->view('vsspd_approval_form_banjar', $data);
            } else
                $this->load->view('vsspd_approval_form', $data);
        } else {
            show_404();
        }
    }
    ...
    public function do_approve()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($id && $sspd = $this->sspd_model->get($id)) {
                $approvalx = $this->sspd_approval_model->get_new_approvalno(date('Y'), '1');
                $tahun     = $approvalx[0];
                $kode      = $approvalx[1];
                $no_urut   = $approvalx[2];
                ...
                $post_data = array(
                    ...
                    'kode' => $kode,
                    'no_urut' => $no_urut,
                    'tgl_approval' => date('Y-m-d', strtotime($input_post['tgl_approval'])),
                    'create_uid' => $this->session->userdata('uid')
                );
                if($this->sspd_approval_model->save($post_data)) {
                    // UPDATE SPPD
                    $upd_sspd = array(
                        'tgl_approval' => date('Y-m-d', strtotime($input_post['tgl_approval'])),
                        'status_daftar' => 3,
                    );
                    $this->sspd_model->update($id, $upd_sspd);
                }

                $this->session->set_flashdata('msg_success', 'Data telah disimpan');
                redirect(active_module_url($this->controller));
            } else {
                $this->session->set_flashdata('msg_error', 'Data SSPD tidak ditemukan dalam database!');
                redirect(active_module_url($this->controller));
            }
        }
    ...
    public function batal() {
        ...
        $approval_id = (int) $this->uri->segment(4);
        if ($approval_id && $approval = $this->sspd_approval_model->get($approval_id)) {
            // hapus SSPD Approval
            $this->sspd_approval_model->delete($approval_id);

            // UPDATE SPPD
            $upd_sspd = array(
                'tgl_approval' => NULL,
                'status_daftar' => 2,  // 3-1 :D
            );
            $this->sspd_model->update($approval->sspd_id, $upd_sspd);
