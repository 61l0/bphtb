...
class ajb extends CI_Controller 
{
	...
	public function index() {
        ...
        $data['tgl_from'] = date('01-m-Y', strtotime('last month'));
        $data['tgl_to'] = date('t-m-Y', strtotime('last month'));;
        
		$this->load->view('vajb', $data);
	}
	...
	function grid() {
        ...
        if($tgl1=='') {
            // $tgl1 = date('Y-m-01', strtotime('last month'));
            // $tgl2 = date('Y-m-t', strtotime('last month'));;
            $tgl1 = date('Y-01-01');
            $tgl2 = date('Y-m-d');;
        } else {
        ...
        // menampilkan data SSPD yang sudah bayar saja.
	...
	function cetak() {
        ...
        $kondisi = " and date(tgl_transaksi) between '{$tgl1}' and '{$tgl2}'
            and no_ajb is not null and tgl_ajb is not null ";
