...
class Sspd_kb extends CI_Controller
{
    private $module     = 'sspd';
    private $controller = 'sspd_kb';
    ...
    public function index()
    {
        ...
        if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE) {
            $this->load->view('vsspd_kb_banjar', $data);
        } else {
            $this->load->view('vsspd_kb', $data);
        }
    }
    ...
    function grid()
    {
        ...
        if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE) {
            ...
            $this->datatables->join('bphtb_validasi v', 'ss.id=v.sspd_id', 'left');
            ...
            $this->datatables->where("cast(ss.kode as integer)=4");
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $sspd_kode = $this->sspd_model->get_new_sspdno(date('Y'), '4');
            $tahun     = $sspd_kode[0];
            $kode      = $sspd_kode[1];
            $no_sspd   = $sspd_kode[2];
        ...
        $data['dt']['tarif']           = ($data['dt']['tarif'] > 0) ? $data['dt']['tarif'] : 5;

        if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE)
            $this->load->view('vsspd_kb_form_banjar', $data);
        else
            $this->load->view('vsspd_kb_form', $data);
    }
    ...
    public function edit()
    {
        ...
        if ($id && $get = $this->sspd_model->get($id)) {
            // khususon kode = 4 :p
            if ((intval($get->status_pembayaran) > 0 || (int)$get->kode <> 4) && !is_super_admin()) {
                $data['mode'] = 'view';
            ...
            if(defined('BPHTB_BANJAR') && BPHTB_BANJAR==TRUE) {
                ...
                $this->load->view('vsspd_kb_form_banjar', $data);
            } else $this->load->view('vsspd_kb_form', $data);
        } else {
            show_404();
        }
    }
    ...
    public function delete()
    {
        ...
        if ($id && $get = $this->sspd_model->get($id)) {
            if (intval($get->status_pembayaran) > 0 || (int)$get->kode > 1) {
