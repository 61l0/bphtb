...
class Ajb_verifikasi extends CI_Controller
{
	private $module = 'pbb_bphtb';
	private $controller = 'ajb_verifikasi';
	...
	public function index() {
		...
		$this->load->view('vajb_verifikasi', $data);
	}
	...
	function grid() {
        ...
        // menampilkan data sspd yg AJBnya tidak null dan BELUM di verifikasi saja
        $this->datatables->where("(ss.status_pembayaran=1 or ss.bphtb_harus_dibayarkan=0)");
        $this->datatables->where("ss.tgl_ajb is not null");
        $this->datatables->where("ss.no_ajb is not null");
        $this->datatables->where("(ss.verifikasi_date is null or ss.verifikasi_uid is null)");
	...
	public function edit() {
		...
		if($id && $get = $this->sspd_model->get($id)) {                        
            ...
            $this->load->view('vajb_verifikasi_form',$data);
		} else {
			show_404();
		}
	}
	...
	public function update() {
		...
		if ($this->form_validation->run()) {
            $dat = array(                
                ...
                'verifikasi_date' => $this->input->post('verifikasi_date') ? date('Y-m-d', strtotime($this->input->post('verifikasi_date'))) : date('Y-m-d'),
                'verifikasi_uid' => $this->session->userdata('uid'),
			);
	...
	function cetak() {
        ...
        $kondisi = " and date(tgl_transaksi) between '{$tgl1}' and '{$tgl2}'         
            and verifikasi_uid is not null
            and (status_pembayaran=1 or bphtb_harus_dibayarkan=0) ";
            // and (verifikasi_bphtb_date is not null or verifikasi_bphtb_uid is not null) ";
