...
$(document).ready(function() {
	oTable = $('#table1').dataTable({
		...
		"aoColumnDefs": [
			...
			{ "aTargets": [8], "bSearchable": true,  "bVisible": true,  "sWidth": "180px", "sClass": "" },
		],
    ...
    var mytoolbar = '<div class="btn-group pull-left">' +
                    ...
                    '<div class="btn-group pull-left">' +
                    '    <div class="input-prepend"><span class="add-on"><i class="icon-calendar"></i></span><input style="width:64px;" id="tgl1" name="tgl1" width="5" type="text" placeholder="dd-mm-yyyy" value="<?=date('d-m-Y', strtotime('last days'));?>"/></div>' +
                    '</div>' +
                    '<div class="btn-group pull-left">' +
	...
	$('#btn_proses').click(function() {
        ...
        if(proses_id != 2) {
            if(mID) 
                window.location = '<?=active_module_url($this->uri->segment(2));?>register/'+mID;
            else
	...
	$('#btn_batal').click(function() {
        ...
        if(proses_id == 2) {
            if(mID) {
                if(confirm('Batalkan SK untuk data ini?')) {
                    $.ajax({
...
<div class="content">
    <div class="container-fluid">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#main_grid"><strong>Penetapan SKPD-KB/LB</strong></a></li>
        </ul>
        <?=msg_block();?>
