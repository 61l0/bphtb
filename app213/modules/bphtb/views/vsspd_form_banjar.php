    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if(parseInt(result['jmltr']) > 0)
                alert ('Ditemukan '+result['jmltr']+' transaksi untuk Identitas ini.');
        },
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if(data['wp_nama'])
                $("#wp_nama_asal").val(data['wp_nama']);
            if(data['wp_npwp'])
                $("#wp_npwp_asal").val(data['wp_npwp']);
            if(data['wp_alamat'])
                $("#wp_alamat_asal").val(data['wp_alamat']);
            if(data['wp_blok_kav'])
                $("#wp_blok_kav_asal").val(data['wp_blok_kav']);
            if(data['wp_rt'])
                $("#wp_rt_asal").val(data['wp_rt']);
            if(data['wp_rw'])
                $("#wp_rw_asal").val(data['wp_rw']);
            if(data['wp_kelurahan'])
                $("#wp_kelurahan_asal").val(data['wp_kelurahan']);
            if(data['wp_kecamatan'])
                $("#wp_kecamatan_asal").val(data['wp_kecamatan']);
            if(data['wp_kota'])
                $("#wp_kota_asal").val(data['wp_kota']);
            if(data['wp_provinsi'])
                $("#wp_provinsi_asal").val(data['wp_provinsi']);
            if(data['wp_kdpos'])
                $("#wp_kdpos_asal").val(data['wp_kdpos']);
...
function calculate_old() {
    ...
    var vx3 = parseInt($("#tarif").autoNumeric('get'));
...
function calculate() {
    ...
    var harga_transaksi = parseFloat($("#harga_transaksi").autoNumeric('get'));
    ...
    var npop=0;
    if(njop <= harga_transaksi)
        npop = harga_transaksi * (bagian / pembagi);
    else
        npop = njop * (bagian / pembagi);
	...
	var terhutang = 0;
	var perolehan = $('#perolehan_id').val();
	// if (npop > njop || perolehan==8)
		// terhutang = parseInt(npop  - npoptkp) * tarif / 100); // terhutang
	// else
		// terhutang = parseInt((njop - npoptkp) * tarif / 100); // terhutang
    terhutang = parseInt(npopkp * tarif / 100); // terhutang
		
    terhutang = (terhutang > 0) ? terhutang : 0;
    $("#terhutang").autoNumeric('set', terhutang);

    var tarif_pengurang = parseInt($("#tarif_pengurang").autoNumeric('get'));
    var pengurang = parseInt(terhutang * tarif_pengurang / 100); // pengurang
    pengurang = (pengurang > 0) ? pengurang : 0;
    ...
    var bphtb_harus_dibayarkan = (terhutang - pengurang ) - bphtb_sudah_dibayarkan + denda; // bphtb yg harus dibayar
...
$(document).ready(function () {    
    ...
    $('#tgl_transaksi, #tgl_jatuh_tempo, #pengurangan_sk_tgl, #ketetapan_tgl, #ketetapan_jatuh_tempo_tgl, #pengurangan_jatuh_tempo_tgl').formatter({
        'pattern': '{{99}}-{{99}}-{{9999}}',
    });
	$('#tgl_transaksi, #tgl_jatuh_tempo, #pengurangan_sk_tgl, #ketetapan_tgl, #ketetapan_jatuh_tempo_tgl, #pengurangan_jatuh_tempo_tgl').datepicker({
		dateFormat:'dd-mm-yy',
		changeMonth:true, 
		changeYear:true,
	});
	...
	$('#npopkp, #harga_transaksi, #npoptkp, #tarif, #terhutang, #bagian, #pembagi, #tarif_pengurang, #pengurang').autoNumeric('init', {
		aSep: '.', aDec: ',', vMax: '999999999999.99',  mDec: '0'
	});
	...
	$('#btn_cari_wp_asal').click(function() {
        var id = $('#wp_identitas_asal').val();
        if(id)
            cari_wp_asal(id);
        else
            alert('Harap mengisi Nomor Identitas WP Asal!')
    });
    
    $('#wp_identitas_asal').keypress(function(event){
        if (event.which == '13') {
            $('#btn_cari_wp_asal').trigger('click');
        }
    });
...
<div class="content">
	<div class="container-fluid">
		<ul class="nav nav-tabs">
			<li class="active">
				<a href="#"><strong><? echo $this->uri->segment(2) == 'sspd' ? 'Registrasi SSPD' : 'Validasi SSPD'; ?></strong></a>
			</li>
		</ul>
		...
		<?php echo form_open($faction, array('id'=>'myform','class'=>'form-horizontal',
			...
			<div class="control-group ">
                ...
                <div class="controls">
                    <input style="width:255px;" class="input-xlarge" type="text" id="no_ajb" name="no_ajb" value="<?=$dt['no_ajb'];?>">
                </div>
			</div>
			<div class="control-group">
					...
					<div class="tab-pane fade in" id="perhitungan">
						<div class="row">
							<div class="span6" style="width: 450px;">
                                ...
                                <div class="control-group">
									<label class="control-label" style="width: 180px">NPOPKP</label>
									<div class="controls">
										<input class="input-small" type="text" id="npopkp" name="npopkp" value="<?= ($dt['npopkp'])?$dt['npopkp']:0;?>" readonly>
									</div>
								</div>
								<div class="control-group">
