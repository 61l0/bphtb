...
$(document).ready(function() {
    oTable = $('#table1').dataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex) {
            $(nRow).on("click", function (event) {
                if ($(this).hasClass('row_selected')) {
                    mID = '';approved='';noSSPD='';jmlBayar=0;
        ...
        "fnServerParams": function ( aoData ) {
            // aoData.push( { "name": "tgl1", "value": $('#tgl1').val() },{ "name": "tgl2", "value": $('#tgl1').val() } );
        },
        ...
        "fnStateLoaded": function (oSettings, oData) {
            ...
            // tgl2= oData.tgl2;
            // alert(tgl1);
            // alert( 'Saved filter was: '+oData.tgl1 );
            // alert(oData.tgl1);
            // alert(oData.tgl2);
            // alert("LOAD PARAMS:\n"+oData.toSource());
            // tgl1= oData.tgl1;
    ...
    var mytoolbar = '<div class="btn-group pull-left">' +
                    ...
                    '<button id="btn_print_form" type="button" data-value="printed" class="btn btn-info"><i class="icon-print icon-white"></i>&nbsp;SSPD</button>' +
                    '<button id="btn_print" type="button" data-value="printed" class="btn btn-info"><i class="icon-print icon-white"></i>&nbsp;SSPD (Ploted)</button>' +
                    '<button id="btn_print_approve" type="button" data-value="printed" class="btn btn-info"><i class="icon-print icon-white"></i>&nbsp;Approved</button>' +
                    <? if (!($this->session->userdata('isppat'))) : ?>
                    '<button id="btn_print_skpdn" type="button" data-value="printed" class="btn btn-info"><i class="icon-print icon-white"></i>&nbsp;SKPDN</button>' +
                    '<button id="btn_print_skpdkb" type="button" data-value="printed" class="btn btn-info"><i class="icon-print icon-white"></i>&nbsp;SKPDKB</button>' +
                    <? endif; ?>
                    '</div>' +
    ...
    <? if (!($this->session->userdata('isppat'))) : ?>
    $('#btn_print_skpdn').click(function() {
        if(mID) {
            if(jmlBayar==0) {
                var rpt = 'sk_nihil';
                var winparams = 'location=1,status=1,scrollbars=1,resizable=no,width='+screen.width+',height='+screen.height+',menubar=no,toolbar=no,fullscreen=no';
    ...
    $('#btn_print_skpdkb').click(function() {
        if(mID) {
            if(noSSPD.substring(5,7)=='02' || noSSPD.substring(5,7)=='04') {
                var rpt = 'skpdkb';
                var winparams = 'location=1,status=1,scrollbars=1,resizable=no,width='+screen.width+',height='+screen.height+',menubar=no,toolbar=no,fullscreen=no';
                window.open('<?=active_module_url($this->uri->segment(2));?>show_rpt/?'+rpt+'/'+mID, 'Laporan', winparams);
            } else {
                alert('Ini bukan data Kurang Bayar !');
            }
        } else {
            alert('Silahkan pilih data yang akan diprint');
        }
...
<div class="content">
    <div class="container-fluid">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#main_grid"><strong>Registrasi SSPD</strong></a></li>
        </ul>
        <?=msg_block();?>
