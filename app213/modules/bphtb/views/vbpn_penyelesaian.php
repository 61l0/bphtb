...
$(document).ready(function() {
	...
	$('#btn_proses').click(function() {
        ...
        if(proses_id != 2) {
            if(mID) 
                window.location = '<?=active_module_url($this->uri->segment(2));?>terima/'+mID;
            else
	...
	$('#btn_batal').click(function() {
        ...
        if(proses_id == 2) {
            if(mID) {
                if(confirm('Pembatalan untuk data ini?')) {
                    $.ajax({
	...
	$("#proses_id").change(function() {
        if ($(this).val() == 1 )
            $("#tgl").html('Tgl. Terima<br>BPN');
        else
            $("#tgl").html('Tgl. Selesai<br>BPN');
...
<div class="content">
    <div class="container-fluid">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#main_grid"><strong>Register Penyelesaian Berkas</strong></a></li>
        </ul>
        <?=msg_block();?>
        <table class="display dataTables" id="table1">
            <thead class="head">
                <tr>
                    ...
                    <th>No.</th>
                    <th>No. SSPD</th>
                    <th id="tgl">Tgl. Terima<br>BPN</th>
                    <th>NOP</th>
                    <th>Thn. SPPT</th>
                    <th>Nama WP</th>
