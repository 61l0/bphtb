...
$(document).ready(function() {
    ...
    var mytoolbar = '<div class="btn-group pull-left">' +
                    ...
                    '<button id="btn_reversal" class="btn btn-primary hide" type="button">Reversal</button>' +
                    '</div>' +
                    ...
                    '<div class="btn-group pull-left">' +
                    '    <div class="input-prepend"><span class="add-on"><i class="icon-calendar"></i></span><input style="width:64px;" id="tgl1" name="tgl1" width="5" type="text" placeholder="dd-mm-yyyy" value="<?=date('d-m-Y');?>"/></div>' +
                    '</div>' +
                    '<div class="btn-group pull-left">' +
	...
	$('#btn_proses').click(function() {
        ...
        if(proses_id != 2) {
            if(mID) 
                window.location = '<?=active_module_url($this->uri->segment(2));?>bayar/'+mID;
            else
	...
	$('#btn_reversal').click(function() {
        var proses_id = $('#proses_id').val();
        if(proses_id == 2) {
            if(mID) {
                if(confirm('Batalkan pembayaran untuk data ini?')) {
                    $.ajax({
                        url: '<?=active_module_url($this->uri->segment(2));?>reversal/'+mID,
                        async: false,
                        success: function (data) {
                            if (data != 'ok') 
                                alert('Reversal gagal');
                            else reload_grid();
                        },
...
<div class="content">
    <div class="container-fluid">
        <ul class="nav nav-tabs">
            <li class="active"><a data-toggle="tab" href="#main_grid"><strong>Daftar Penerimaan</strong></a></li>
        </ul>
        <?=msg_block();?>
