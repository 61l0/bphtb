...
class Bphtb_model extends CI_Model
{
    ...
    function get_npoptkp($id) { // ??
        $sql = "select * from bphtb_perolehan where id = ?";
    ...
    function informasi_objek_pajak($nop) {
        ...
        $sql = " select
                dop.kd_propinsi||'.'||dop.kd_dati2||'-'||dop.kd_kecamatan||'.'||dop.kd_kelurahan ||'-'|| dop.kd_blok ||'.'||dop.no_urut||'.'|| dop.kd_jns_op nop,
            ...
            where dop.kd_propinsi = ?
                and dop.kd_dati2 = ?
                and dop.kd_kecamatan = ?
                and dop.kd_kelurahan = ?
                and dop.kd_blok = ?
                and dop.no_urut = ?
                and dop.kd_jns_op = ?
