...
class loaddata extends CI_Controller
{
    ...
    function transaksi1()
    {
        ...
        $sql_query_r = "
          SELECT  
            ...
            k.nm_wp_sppt uraian, 
            k.pbb_yg_harus_dibayar_sppt pokok, p.denda_sppt denda, p.jml_sppt_yg_dibayar bayar, to_char(p.tgl_pembayaran_sppt,'dd-mm-yyyy') tanggal
          FROM sppt k 
          INNER JOIN pembayaran_sppt p 
            ON k.kd_propinsi = p.kd_propinsi
            AND k.kd_dati2 = p.kd_dati2 
        ...
        $row[]              = '';
        $output['aaData'][] = $row;
        //echo" iTotalDisplayRecords" . $iFiltered . 
        //  " iDisplayStart" . $iDisplayStart .
        //  " iDisplayLength" . $iDisplayLength;
        
        //echo (strval($iDisplayStart+$sLimit+1));
        //die(' a '.$iFiltered);
    ...
    function tranmonths()
    {
        ...
        $sLimit = "";
        //$pageSize=(isset($_GET['iDisplayLength']) && $_GET['iDisplayLength']!='-1'?$_GET['iDisplayLength'] : 15);
        //$pageNumber=(isset($_GET['iDisplayStart']) && $_GET['iDisplayStart']!='-1'?$_GET['iDisplayStart'] : 15);
        if (isset($_GET['iDisplayStart']) && $_GET['iDisplayLength'] != '-1') {
            $sLimit = "LIMIT " . $_GET['iDisplayLength'] . " OFFSET " . $_GET['iDisplayStart'];
        }
    ...
    function piutang()
    {
        ...
        if ($kec_kd == '000') {
            ...
            $sql_query_c = "
				SELECT COUNT(*) AS c 
				FROM ( 
          SELECT k.kd_propinsi, k.kd_dati2, k.kd_kecamatan, k.kd_kelurahan, k.kd_blok, k.no_urut
