...
class user_pbbms_model extends CI_Model {
	...
	function get_all()
	{	$sql = "
select u.nama, u.disabled, u.created, up.user_id, up.kd_propinsi, up.kd_dati2, up.kd_kecamatan, up.kd_kelurahan, kel.nm_kelurahan, kec.nm_kecamatan
from users u 
...
order by u.disabled desc, u.nama";
	...
	function get_by_kec_kel($kec_kd, $kel_kd) {
		$sql = "SELECT u.* 
			FROM users u 
			INNER JOIN user_pbbms up ON u.id=up.user_id
			WHERE up.kd_kecamatan='{$kec_kd}' AND up.kd_kelurahan='{$kel_kd}'";
		$query = $this->db->query($sql);
		if($query->num_rows()!==0)
		{
			return $query->row();
