...
class pbbm_model extends CI_Model {
  ...
  function __construct() {
    parent::__construct();
    $this->rangebuku=array(1=>array(0=>0,1=>100000),
                     2=>array(0=>100001, 1=>500000),
                     3=>array(0=>500001, 1=>2000000),
                     4=>array(0=>2000001,1=>5000000),
                     5=>array(0=>5000001,1=>9999999999));
    }
  function getKodeBlok() {return $this->kode_blok;}
  function setKodeBlok($kode_blok) {$this->kode_blok = $kode_blok;}
  function getKodePropinsi() {return $this->kode_propinsi;}
  function setKodePropinsi($kode_propinsi) {$this->kode_propinsi = $kode_propinsi;}
  function getKodeDati2() {return $this->kode_dati2;}
  function setKodeDati2($kode_dati2) {$this->kode_dati2 = $kode_dati2;}
  function getKodeJenisOP() {return $this->kode_jenis_op;}
  function setKodeJenisOP($kode_jenis_op) {$this->kode_jenis_op = $kode_jenis_op;}
  function getKodeKecamatan() {return $this->kode_kecamatan;}
  function setKodeKecamatan($kode_kecamatan) {$this->kode_kecamatan = $kode_kecamatan;}
  function getKodeKelurahan() {return $this->kode_kelurahan;}
  function setKodeKelurahan($kode_kelurahan) {$this->kode_kelurahan = $kode_kelurahan;}
  function getNOP() {return $this->nop;}
  function setNOP($nop) {$this->nop = $nop;}
  function getNoUrut() {return $this->no_urut;}
  function setNoUrut($no_urut) {$this->no_urut = $no_urut;}
  function getNamaWP() {return $this->nama_wp;}
  function setNamaWP($nama_wp) {$this->nama_wp = $nama_wp;}
    ...
    $sql = "SELECT 
        ...
        $sql .= " GROUP BY s.kd_propinsi, s.kd_dati2, s.kd_kecamatan, s.kd_kelurahan, 
                  ...
                  ORDER BY s.kd_propinsi, s.kd_dati2, s.kd_kecamatan, s.kd_kelurahan, 
                        s.kd_blok, s.no_urut, s.kd_jns_op, s.thn_pajak_sppt DESC LIMIT 10";
    ...
    $r= "
        ...
        FROM (
            ...
            WHERE k.kd_propinsi='".KD_PROPINSI."' AND k.kd_dati2='".KD_DATI2."'
                  and p.thn_pajak_sppt='$tahun' 
                  AND s.pbb_yg_harus_dibayar_sppt between $bukumin AND $bukumax
                  AND tgl_pembayaran_sppt >= '$tglm' AND tgl_pembayaran_sppt < '$tgls' 
                  AND p.kd_propinsi = s.kd_propinsi AND p.kd_dati2 = s.kd_dati2 AND p.kd_kecamatan = s.kd_kecamatan AND p.kd_kelurahan = s.kd_kelurahan 
                  AND p.kd_blok = s.kd_blok 
                  AND p.no_urut = s.no_urut AND p.kd_jns_op = s.kd_jns_op AND p.thn_pajak_sppt = s.thn_pajak_sppt 
                  AND k.kd_propinsi = s.kd_propinsi AND k.kd_dati2 = s.kd_dati2 AND k.kd_kecamatan = s.kd_kecamatan 
...
function qry_realisasi_lb_kec($tahun){
    return "
        SELECT  k.kd_propinsi||'.'||k.kd_dati2||'-'||k.kd_kecamatan kode, k.nm_kecamatan uraian,
                count(s.kd_propinsi) sppt1, sum(amount1) amount1,  sum(amount2) amount2, sum(amount1) - sum(amount2) amount3 
        FROM ref_kecamatan k
        LEFT JOIN (
